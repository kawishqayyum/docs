...
<Note>
可以通过 [API](/api-reference/account-management-api/Switch_100_uptime_ON_OFF_in_a_Static_zone) 开启或关闭 100% 正常运行时间功能。
</Note>
...
* **通过 API 终端节点：**
	+ [将 IP 添加到区域白名单](/api-reference/account-management-api/Add_IP_to_Zone_whitelist)
	+ [从区域白名单中移除 IP](/api-reference/account-management-api/Remove_IP_from_Zone_whitelist)
	+ [将 IP 添加到区域黑名单](/api-reference/account-management-api/Add_IP_to_Zone_blacklist)
	+ [从区域黑名单中移除 IP](/api-reference/account-management-api/Remove_IP_from_Zone_blacklist)
	+ [从区域白名单/黑名单中移除域名](/api-reference/account-management-api/Remove_domain_from_Zone_whitelist_blacklist)
...
* 您的机器 IP 未列入白名单。 要了解如何将 IP 列入白名单，请查阅以下文章。
  https://docs.brightdata.com/api-reference/proxy-manager/whitelist_ips
