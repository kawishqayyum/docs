...
The zone name is used to create your proxy username, which you can then [customise if you want in order to control your proxies](https://docs.brightdata.com/cn/proxy-networks/config-options). Here is a sample proxy username for proxies in the USA - as you can see it includes the zone name inside it:
...
Bright Data可以轻松集成到许多第三方工具中。 通常，只需使用您的Bright Data产品凭据来配置HTTP/HTTPS代理即可。 If you are using residential proxies, web unlocker or SERP API don't forget to [use the SSL certificate](https://docs.brightdata.com/cn/general/account/ssl-certificate) to ensure and to end encrypted connections.
...
Remember, you can also choose proxies by country, state, city, zip code and ASN. See [this guide](https://docs.brightdata.com/cn/proxy-networks/config-options) for more info.
...
In order to target a specific city you will need to use a [共享IP (pay-per-IP)](/cn/proxy-networks/data-center/configure-your-proxy#ip-type) or [专用IP](/cn/proxy-networks/data-center/configure-your-proxy#ip-type) proxy network.
...
<Note>
100% uptime feature can be turned on or off, [via API](/cn/api-reference/account-management-api/Switch_100_uptime_ON_OFF_in_a_Static_zone).
</Note>
...
* **通过API端点：**
	+ [将IP添加到区域白名单](/cn/api-reference/account-management-api/Add_IP_to_Zone_whitelist)
	+ [将IP从区域白名单中移除](/cn/api-reference/account-management-api/Remove_IP_from_Zone_whitelist)
	+ [将IP添加到区域黑名单](/cn/api-reference/account-management-api/Add_IP_to_Zone_blacklist)
	+ [将IP从区域黑名单中移除](/cn/api-reference/account-management-api/Remove_IP_from_Zone_blacklist)
	+ [将域名从区域白名单/黑名单中移除](/cn/api-reference/account-management-api/Remove_domain_from_Zone_whitelist_blacklist)
...
Alternatively, you can use API to refresh your [dedicated 住宅 IPs](/cn/api-reference/account-management-api/Refresh_dedicated_residential_IPs) or your [Datacenter/ISP IPs](/cn/api-reference/account-management-api/Refresh_Static_Datacenter_ISP_IPs)
...
专用IP 住宅 IPs can be selected in the form of [gIPs](/cn/proxy-networks/residential/configure-your-proxy#ip-groups-gips). They can be allocated under the zone's configuration page by selecting a "专用IP" IP type and choosing a number of gips. 此外，还需要针对特定域名。
...
#### 用于单步抓取：
[SERP API](https://docs.brightdata.com/cn/scraping-automation/serp-api/introduction) is the ideal product for targeting SERPs as it has a guaranteed success rate (pay only for success) with active unlocking, automatically chooses the best proxies, customizes headers, fingerprinting, solves CAPTCHAs, and more. 
...
* 您的机器IP未列入白名单。 In order to learn how to whitelist an IP, please review the [following article.](/cn/api-reference/proxy-manager/whitelist_ips)
...
For more info on what 'Immediate Access' mode means: [Immediate Access Explanation](https://docs.brightdata.com/cn/proxy-networks/residential/network-access)
...
- Install the BrightData SSL certificate, according to our [SSL certificate installation guide](https://docs.brightdata.com/cn/general/account/ssl-certificate) (note that for some specific integration methods installing the ssl certificate may not be possible due to it not being supported)
- If you are utilizing our proxies through code (NodeJS, Python, C#, etc.)  you can ignore SSL verification entirely, which resolves all SSL verification errors. We provide examples on how to do this in the [SSL certificate documentation page](https://docs.brightdata.com/cn/general/account/ssl-certificate)
</Accordion>
